...
from pathlib import Path
from MasterProject.utils.utils import stack_trj, make_inpt_seq
...
class Model(nn.Module):
    def __init__(self, model_setup, opt_lr, writer, iterations=1, threshold = 0) -> None:
        ...
        self.critic_transformer = TransformerModel(model_setup=self.model_setup['transformer'])
    ...
    def get_trj_size(self, task_embedding):
        return (task_embedding.size(0), self.trj_len, self.model_setup['decoder']['d_output'])


    def get_critic_score(self, task_embedding, last_seq, detach):
        if detach:
            last_seq = last_seq.detach()
        seq_trans = self.get_seq_crit(task_embedding=task_embedding, last_seq=last_seq)
    ...
    def get_critic(self, seq_transformer):
        #critic_score = self.critic(seq_transformer[:,-1]).squeeze() #N,1
        critic_score = seq_transformer[:,-1,0]
        return critic_score
    ...
    def optimize(self, task_embedding):
        ...
        trj_optimizer = torch.optim.AdamW([opt_trj], lr=1e-3)
        critic_score = self.get_critic_score(task_embedding=task_embedding, last_seq=opt_trj, detach=False)
